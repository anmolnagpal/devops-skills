...
- **agentskills.io conformance, enforced.** `scripts/check-agentskills.sh` (and a new CI gate) assert every skill meets the [agentskills.io](https://agentskills.io) open Agent Skills standard: `name` + `description` present, name lowercase-kebab ≤ 64 chars matching its directory and free of reserved vendor words, description ≤ 1024 chars, and no angle brackets in any frontmatter value. `scripts/agentskills-skill.schema.json` carries the schema. All 18 skills conform, so the repo can be listed on agentskills.io.
...
- **Framework mappings on 14 skills.** A `frameworks:` block in each security and infra skill's frontmatter maps it to MITRE ATT&CK (Enterprise), NIST CSF 2.0, and MITRE D3FEND. Two generated artifacts fall out of it: `index.json` (a machine-readable skill catalog) and `FRAMEWORKS.md` (coverage tables, technique to skills), both produced by the new `scripts/generate-index.py` and guarded by `scripts/generate.sh --check` the same way the Cursor/Codex adapters are. Across the 15 mapped skills: 24 ATT&CK techniques, 27 NIST CSF 2.0 subcategories, 15 D3FEND techniques.
- **A `logging` skill** — a read-only reviewer for security-relevant logging posture across Kubernetes and the cloud control plane: is the audit/access record on, complete, shipped off the node that wrote it, and tamper-resistant. Nine `LOG-*` rules span generic Kubernetes audit policy (including k3s/RKE2 `config.yaml`), GKE control-plane and Data Access logging, AKS diagnostic settings, AWS CloudWatch audit-log encryption, and cross-cloud sink immutability. It stays in its lane: AWS CloudTrail/flow-log/EKS-audit/S3-access IaC remains `tf`'s `SEC-LOG-*`, and retention/operability remains `observability`'s `OBS-LOG-*`. Ships 13 eval fixtures (bad and clean per platform); `LOG-K8S-001` was live-validated against a real k3s cluster.
- **Audit-trail integrity rules in `tf`.** `SEC-LOG-003` (a CloudTrail without `enable_log_file_validation`, or not multi-region), `SEC-LOG-004` (a repo-owned audit-log S3 bucket with no versioning and no Object Lock, so the trail is deletable), `SEC-LOG-005` (an `aws_eks_cluster` whose `enabled_cluster_log_types` omits `audit`), and `SEC-LOG-006` (an internet-facing ALB or internet-exposed S3 bucket with no access logging). These enforce the `T1562.008`/`T1070` techniques the skills are now mapped to but did not previously check.
- **README refresh.** A headline metric line, skills/rules/eval/framework badges, and a "Mapped to the frameworks" section that surfaces the coverage counts and links `FRAMEWORKS.md` and `index.json`.

- **A `workflow_dispatch` workflow for the behavioral evals** (`.github/workflows/behavioral-evals.yml`), defaulting to the trigger suite. Deliberately not scheduled: Tier-2 is run manually, triggers-first, and `_docs/EVAL-RESULTS.md` records both the reasoning and the cost that accepts. Takes either `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`, uses a Claude subscription, no API account) or `ANTHROPIC_API_KEY`, and skips with a warning without either. Installs the plugin from the checkout, since the harness refuses a copy that does not match the tree.
- **`--repeat N` for pass@N.** Model output is non-deterministic, so one failing run cannot distinguish a flake from a regression. A case failing in some passes is a flake; one failing every pass is a regression.
- **The `planned` worklist is empty.** Every one of the 166 registry IDs is now either emitted by a skill (160) or declared `reserved` because it cannot be checked from files (6: five DNS/email records and one git-history scan). The final five: `CICD-FLOW-001` in both `github-actions` and `ci` (a deploy that does not depend on a *passing* test job, which stage ordering alone does not provide), `CICD-SCAN-002` and `CICD-SCAN-003` in `github-actions` (no dependency scanning; an image built and pushed with no scan between), and `ARCH-HA-002` plus `ARCH-SCAL-001` in `k8s` (no autoscaling on a request-serving workload; resources shaped so the only way to take load is a bigger pod).
- **Twelve rules implemented in total**, taking the `planned` worklist from 17 to 0. `appsec` gains `SEC-SEC-003` (a committed `.env` holding real values) and `SEC-SEC-004` (committed private key material), where the bad and clean fixtures differ only by a filename suffix. `tf` gains `ARCH-SPOF-001` (single-instance database in staging or prod), `TF-RES-002` (stateful resource with no `lifecycle` guard), `SEC-NET-003` (no subnet tiering), `COST-K8S-002` (on-demand-only node group), and `TF-QUAL-002` (module directory with no README, in a repo-shaped fixture since the rule is about a directory).
- **False-positive exclusions for `finops` and `deploy`**, the last two review skills without any. `finops` gets six, and states the inversion that matters: a cost finding is an opportunity, so an uncertain one should be reported with its uncertainty rather than suppressed, which is the opposite of the review skills. `deploy` gets five, the first being that it must never substitute its own reading of an artifact for the skill that owns it, since that is how a gate starts disagreeing with the skills it aggregates.
- **`.gitignore` now covers private key material** (`*.pem`, `*.key`, `*.p12`, `*.pfx`, `*.jks`, `id_rsa`, `id_ed25519`, `id_ecdsa`). This repo ships a skill that flags a committed private key and had no rule against committing one itself.
...
- **Install on macOS default bash 3.2 (#11).** macOS ships only bash 3.2, where expanding an empty array as `"${arr[@]}"` under `set -u` errors with "unbound variable" (fixed upstream in bash 4.4). `install.sh` and `bootstrap.sh` both hit it whenever no optional flags were passed, aborting every macOS install. Both call sites now use `${arr[@]+"${arr[@]}"}`, which expands to nothing on an empty array. `bootstrap.sh` also stopped trusting `[ -r /dev/tty ]`, which passes in non-interactive SSH sessions on macOS where the subsequent open fails with "Device not configured"; it now tests by actually opening `/dev/tty`, and an empty tool selection is rejected with a clear message instead of an array crash.
- **`SEC-SEC-002` moved from `planned` to `reserved`.** A git-history scan cannot be tested by fixture: an eval case directory sits inside this repo's history and cannot carry its own. Implementing it would have broken the property that every catalog rule has a fixture behind it, so it is now declared unfixturable with that reason, alongside the DNS and email rules.
- **The harness content hash now covers `references/` and skill-root `.md` files**, not just `SKILL.md`. Those ship with the plugin and can drift the same way.
