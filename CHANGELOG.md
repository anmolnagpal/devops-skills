...
- **Git Data API branch/commit/PR preparation in `github`**: documented the blob → tree → commit → ref → PR flow for sandboxes where raw `git` write subcommands require approval that never resolves headlessly, including the `-F` (not `-f`) requirement for reading file content into a blob, extended to a rebase-an-existing-PR-onto-a-moved-base technique, and a check-for-existing-work precondition before redoing a requeued/retried job.
- **`CICD-OPS-007` in `github-actions`**: flags a watchdog, monitoring, or gate workflow that runs only on the same self-hosted infra it is meant to monitor or protect (for example, a runner-health check scheduled on the runner pool it watches), with a new eval case. Left uncaught, an outage of that pool either silently disables the check or deadlocks the PR that would fix the outage.
- **Retry-loop stderr hygiene in `github-actions`**: a bash retry loop around `if VAR=$(cmd); then` must never merge stderr into the same substitution used for the success value — a passing command's non-fatal stderr silently corrupts the captured result.
- **Self-hosted/ARC runner tool-availability checklist in `github-actions`**: migrating a workflow from `ubuntu-latest` to a self-hosted/ARC runner needs an explicit check that every tool its `run:` steps assume (`gh`, `jq`, `docker`, language runtimes) is actually present on the target image, plus a validated `gh` CLI install pattern for images that lack it.
