...
- **Tier-2 behavioral evals now run weekly in CI** (`.github/workflows/behavioral-evals.yml`) and on demand, so a description edit that breaks routing surfaces within a week rather than never. Skips with a warning when `ANTHROPIC_API_KEY` is absent, installs the plugin from the checkout (the harness refuses a copy that does not match the tree), and files an issue on a scheduled failure telling the reader to check the eval before the skill.
