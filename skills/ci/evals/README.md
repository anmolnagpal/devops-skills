# GitLab CI skill evals

Eval-driven development for the `/ci` review mode. Each case is an **input
...
evals/
  ...
  cases/
    <case-name>/
      <input file>     ← .gitlab-ci.yml fixture
...
2. **Model grading (manual / harness).** Run the `/ci review` skill against
...
```bash
bash skills/ci/evals/validate.sh
```
...
2. Drop a fixture (a `.gitlab-ci.yml`) that violates (or cleanly
